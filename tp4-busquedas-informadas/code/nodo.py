...
class Nodo:
    ...
    def random_actions(self, env, action):
        long = env.size
        value = True
        if action == 0:
            if self.estado[1] - 1 >= 0 and env.map_list[self.estado[0]][self.estado[1] - 1] != 'H': 
                pos = (self.estado[0], self.estado[1] - 1)
            else: 
                value = False
                pos = (0,0)
        elif action == 1:
            if self.estado[0] + 1 < long and env.map_list[self.estado[0] + 1][self.estado[1]] != 'H': 
                pos =(self.estado[0]+1, self.estado[1])
            else: 
                value = False
                pos = (0,0)                
        elif action == 2:
